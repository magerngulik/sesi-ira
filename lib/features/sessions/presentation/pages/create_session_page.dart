...
class _CreateSessionPageState extends State<CreateSessionPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Booking Session')),
      body: DecoratedBox(
          ...
          child: FutureBuilder<_CreateSessionOptions>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasError) {
                return StateMessage(
                  title: 'Form booking belum bisa dibuka',
              ...
              final isFirstSession = options.nextSessionNumber == 1;
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          if (_currentStep == 0)
                            _buildBookingInfoStep(
                              options: options,
                              isFirstSession: isFirstSession,
                            )
                          else
                            _buildBookingNotesStep(),
                          const SizedBox(height: 24),
  ...
  Widget _buildBookingInfoStep({
    required _CreateSessionOptions options,
    required bool isFirstSession,
  }) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
        const _SectionTitle(title: 'Informasi Booking'),
        const SizedBox(height: 8),
        Text(
          'Tahap ini dipakai untuk booking awal sesi. Status session dikunci ke Scheduled sampai pertemuan berlangsung.',
        ...
        _ReadOnlyField(
          ...
          value: isFirstSession
              ? 'Scheduled (Session 1 wajib dimulai dari booking)'
              : 'Scheduled',
        ),
      ],
    );
      ...
      children: <Widget>[
        ...
        TextFormField(
          ...
          decoration: const InputDecoration(
            ...
            hintText: 'Tulis hal khusus yang perlu diperhatikan sebelum sesi',
          ),
        ),
      ],
    );
  ...
  Future<void> _submit(_CreateSessionOptions options) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context)
        ..hideCurrentSnackBar()
        ..showSnackBar(
          const SnackBar(content: Text('Booking session berhasil dibuat.')),
        );
    } catch (error) {
      if (!mounted) {
        return;
...
class _CreateSessionOptions {
  const _CreateSessionOptions({required this.nextSessionNumber});
...
class _SessionStepIndicator extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const titles = <String>['Booking', 'Catatan Awal'];
